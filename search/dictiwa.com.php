...
$html = @file_get_contents("https://dictiwa.com/") or die($null);
$dom = new DOMDocument;
@$dom->loadHTML($html);
/*
 * This code will download every conversion types.
   $kind = [];
...
foreach($kind as $k) {
	...
	{
            $h3a = $div->getElementsByTagName("h3")[0]->
	    getElementsByTagName("a")[0];
            $title = filter_var($h3a->nodeValue,
				FILTER_SANITIZE_STRING);
            $link = filter_var($h3a->getAttribute("href"),
			       FILTER_SANITIZE_STRING);
            $desc = filter_var($div->getElementsByTagName("div")[0]->
		getElementsByTagName("p")[0]->nodeValue,
			       FILTER_SANITIZE_STRING);
