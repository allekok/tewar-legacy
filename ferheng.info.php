...
for($pg=1; $pg<=$pgs; $pg++) {
    ...
    foreach($dom->getElementsByTagName("article") as $art) {
        ...
        $title = filter_var($h2->nodeValue, FILTER_SANITIZE_STRING);
        $link = filter_var($h2->getElementsByTagName("a")[0]->getAttribute("href"), FILTER_SANITIZE_STRING);
        $desc = filter_var($art->getElementsByTagName("div")[0]->nodeValue, FILTER_SANITIZE_STRING);
        $desc = mb_strlen($desc) > 150 ? mb_substr($desc, 0, 150) . "..." : $desc; 
        
        if( stristr($link, "alfba") ) {
            $res[] = [
                "stristr" => stristr($title, $q0) or stristr($q0, $title),
